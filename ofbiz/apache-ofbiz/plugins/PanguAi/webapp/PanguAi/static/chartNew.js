...
JSGantt.TaskItem = function(pID, pName, pStart, pEnd, pColor, pLink, pMile, pRes, pComp, pGroup, pParent, pOpen, pDepend, pCaption)
{
      ...
      this.getDuration = function(vFormat){ 
         if (vMile) 
            ...
            {
                tmpPer =   Number(((this.getEnd() - this.getStart()) / (60 * 60 * 1000)).toFixed(1));
                console.log(this.getEnd(),this.getStart())
                console.log(tmpPer)
                if(tmpPer == 1)  
                    vDuration = '1 Hour';
                else
   ...
   this.Draw = function()
   {
      ...
      if(vTaskList.length > 0)
      {
         ...
         vLeftTable +=
            ...
            '  <TD style="BORDER-TOP: #efefef 1px solid; WIDTH: 15px; HEIGHT: 3rem"></TD>' +
            ...
            for(i = 0; i < vTaskList.length; i++)
            {
               ...
               if( vTaskList[i].getGroup()) {
                  if( vTaskList[i].getOpen() == 1) 
                     vLeftTable += '<SPAN id="group_' + vID + '" style="color:#000000;display:none; cursor:pointer; font-weight:bold; FONT-SIZE: 12px;" onclick="JSGantt.folder(' + vID + ','+vGanttVar+');'+vGanttVar+'.DrawDependencies();">&ndash;</span><span style="color:#000000">&nbsp</SPAN>' ;
                  else
               ...
               if(vShowRes ==1) vLeftTable += '  <TD class=gname style="WIDTH: 60px; HEIGHT: 3rem; TEXT-ALIGN: center; BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid;" align=center><NOBR>' + vTaskList[i].getResource() + '</NOBR></TD>' ;
               if(vShowDur ==1) vLeftTable += '  <TD class=gname style="WIDTH: 60px; HEIGHT: 3rem; TEXT-ALIGN: center; BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid;" align=center><NOBR>' + vTaskList[i].getDuration(vFormat) + '</NOBR></TD>' ;
               if(vShowComp==1) vLeftTable += '  <TD class=gname style="WIDTH: 60px; HEIGHT: 3rem; TEXT-ALIGN: center; BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid;" align=center><NOBR>' + vTaskList[i].getCompStr()  + '</NOBR></TD>' ;
               if(vShowStartDate==1) vLeftTable += '  <TD class=gname style="WIDTH: 60px; HEIGHT: 3rem; TEXT-ALIGN: center; BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid;" align=center><NOBR>' + JSGantt.formatDateStr( vTaskList[i].getStart(), vDateDisplayFormat) + '</NOBR></TD>' ;
               if(vShowEndDate==1) vLeftTable += '  <TD class=gname style="WIDTH: 60px; HEIGHT: 3rem; TEXT-ALIGN: center; BORDER-TOP: #efefef 1px solid; FONT-SIZE: 12px; BORDER-LEFT: #efefef 1px solid;" align=center><NOBR>' + JSGantt.formatDateStr( vTaskList[i].getEnd(), vDateDisplayFormat) + '</NOBR></TD>' ;
...
for(j=0;j<24;j++){
...

         {
	       ...
	       if (vFormat=='hour')
	       {
		let vMaxDatePlusOne = new Date(vMaxDate);
			vMaxDatePlusOne.setDate(vMaxDatePlusOne.getDate() + 1);
			vMaxDatePlusOne.setHours(0, 0, 0, 0); 
			console.log(vMaxDatePlusOne)
                vNumUnits = ((vTaskList[i].getEnd()>vMaxDate?vMaxDatePlusOne :vTaskList[i].getEnd()) - vTaskList[i].getStart()) / (60 * 1000) + 1;
                console.log("It is working " + Math.ceil((vNumUnits) * (0.833)-2 ))
	       }
	       else if (vFormat=='minute')
	       {
            ...
            if( vTaskList[i].getMile()) {
               ...
               vTaskRight = 1
               console.log(Math.ceil((vTaskLeft * (vDayWidth) + 1)))
  	            vRightTable +=
