    ...
    <#list orderList as item>
      <div class="col">
        <div class="card h-100 shadow-sm border-0">
          <div class="card-body">
            <h5 class="card-title text-primary">Item : ${item.product}</h5>
            <p class="card-text">
              <strong>Category:</strong> ${item.orderType}<br>
              <strong>Quantity:</strong>${item.quantity}  <br>
              <strong>Location:</strong> ${item.customerName}<br>
              <strong>Order Date:</strong> ${item.createdTxStamp}
            </p>
          </div>
