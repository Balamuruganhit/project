document.addEventListener("DOMContentLoaded", () => {
  ...
  const needsTextarea = cells[1].innerText.trim();
  const expectationsTextarea = cells[3].innerText.trim();
 const objectivesTextarea = cells[4].innerText.trim();
  const needs = needsTextarea?.split('.') || [];
  const expectations = expectationsTextarea?.split('.')
  .map(s => s.trim())
  .filter(s => s !== '') || [];
  const objectives = objectivesTextarea?.split('.')
  .map(s => s.trim())
  .filter(s => s !== '') || [];
    tableData.push({
      issue: party,
      riskDescription:needs ,
      riskValue: type,
  ...
  console.log(tableData);
   const payload = JSON.stringify({ OrganisationDetail : tableData });
  
      // Prepare the XHR request
      const xhr = new XMLHttpRequest();
      xhr.open("POST", "/AuditReport/control/createOrganisationReport", true);
